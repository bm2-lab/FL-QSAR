...
for filename in file_list:
    for count in range(10):
        ...
        train_1,train_2,train_3 = random_split(train,[len(train)//3,len(train)//3,len(train)-2*(len(train)//3)])
        ...
        train_loader_list = [train_loader_1,train_loader_2,train_loader_3]
        ...
        f=open(f'./3_result1/{filename}_result_{count+1}.txt','w')
        torch.manual_seed(1)
        ...
        solo_optimizers=[solo_bobs_optimizer, solo_alices_optimizer,solo_rays_optimizer]
        solo_models = [solo_bobs_model, solo_alices_model,solo_rays_model]
        params = [list(solo_bobs_model.parameters()), list(solo_alices_model.parameters()),list(solo_rays_model.parameters())]
        for i in range(180):
        ...
        f=open(f'./3_result/{filename}_result_{count+1}.txt','w')
        solo_bobs_model = Net().to(device)
        solo_alices_model = Net().to(device)
        solo_rays_model = Net().to(device)
        ...
        for i in range(180):
            ...
            for param_i in range(len(params[0])):
                ...
                new_param = ((spdz_params[0] + spdz_params[1]+spdz_params[2])/3).get_plain_text()
