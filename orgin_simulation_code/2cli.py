...
file_list=['METAB','LOGD','NK1','OX1','OX2','PGP','PPB','RAT_F','TDI','THROMBIN','CB1','DPP4','HIVINT','HIVPROT','3A4']
...
for filename in file_list:
    ...
    for count in range(10):
        ...
        train_1,train_2 = random_split(train,[len(train)//2,len(train)-len(train)//2])
        train_loader_1 = DataLoader(train_1, batch_size=BATCH_SIZE)
        train_loader_2 = DataLoader(train_2, batch_size=BATCH_SIZE)
        train_loader_list = [train_loader_1,train_loader_2]
        ...
        f=open(f'./2_result1/{filename}_result_{count+1}.txt','w')
        torch.manual_seed(1)
        ...
        solo_optimizers=[solo_bobs_optimizer, solo_alices_optimizer]
        solo_models = [solo_bobs_model, solo_alices_model]
        params = [list(solo_bobs_model.parameters()), list(solo_alices_model.parameters())]
        for i in range(180):
            new_params = list()
        ...
        f=open(f'./2_result/{filename}_result_{count+1}.txt','w')
        solo_bobs_model = Net().to(device)
        solo_alices_model = Net().to(device)
        solo_bobs_optimizer = optim.SGD(solo_bobs_model.parameters(), lr=0.05,momentum=0.9,weight_decay=0.0001)
        solo_alices_optimizer = optim.SGD(solo_alices_model.parameters(), lr=0.05,momentum=0.9,weight_decay=0.0001)
        ...
        for i in range(180):
            ...
            for param_i in range(len(params[0])):
                ...
                new_param = ((spdz_params[0] + spdz_params[1])/2).get_plain_text()
